...
class SimpleChat {
  ...
  async inviteToChat(participantId: string): Promise<void> {
    ...
    if (commitResult.commit.wireformat === "mls_public_message") {
      await this.syncGroupState(commitResult.commit as MlsPublicMessage, [
        participantId,
  ...
  async sendMessage(senderId: string, message: string): Promise<void> {
    ...
    const messageResult = await sender.marmot.createMessage(
      sender.groupState,
      message,
    );
    ...
    for (const [participantId, participant] of this.participants) {
      if (participantId !== senderId && participant.groupState) {
        try {
          ...
          console.error(
            `❌ Failed to deliver message to ${participant.name}:`,
            error,
          );
        }
      }
  ...
  private async syncGroupState(
    publicMessage: MlsPublicMessage,
    excludeIds: string[],
  ): Promise<void> {
    ...
    for (const [participantId, participant] of this.participants) {
      if (!excludeIds.includes(participantId) && participant.groupState) {
        try {
          ...
          console.error(
            `❌ Failed to sync group state for ${participant.name}:`,
            error,
          );
        }
      }
...
async function runChatExample(): Promise<void> {
  ...
  try {
    ...
    await chat.sendMessage(
      "alice",
      "Great! This chat is end-to-end encrypted using MLS.",
    );
    await new Promise((resolve) => setTimeout(resolve, 100));
