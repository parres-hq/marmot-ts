...
async function createParticipant(name: string, publicKey: string): Promise<Participant> {
...
async function fullWorkflowExample() {
  ...
  try {
    ...
    const charlie = await createParticipant("Charlie", "charlie_public_key_feedface")

    console.log(`✅ Created participants: ${alice.name}, ${bob.name}, ${charlie.name}\n`)
    ...
    console.log(`✅ Group created with ID: ${Buffer.from(alice.groupState.groupContext.groupId).toString('hex').substring(0, 16)}...\n`)
    ...
    const bobCommitResult = await alice.marmot.createCommit(alice.groupState, [addBobProposal])
    ...
    const aliceMessageResult = await alice.marmot.createMessage(alice.groupState, "Hello Bob!")
    ...
    const bobProcessAlice = await bob.marmot.processMessage(bob.groupState, aliceEncodedMessage)
    if (bobProcessAlice.kind === "newState") {
      throw new Error("Expected application message")
    }
    bob.groupState = bobProcessAlice.newState
    const bobReceivedMessage = new TextDecoder().decode(bobProcessAlice.message)
    console.log(`📥 Bob received: "${bobReceivedMessage}"`)

    // Bob responds
    console.log("📤 Bob sends: 'Hi Alice! Thanks for adding me to the group!'")
    const bobMessageResult = await bob.marmot.createMessage(bob.groupState, "Hi Alice! Thanks for adding me to the group!")
    ...
    const aliceProcessBob = await alice.marmot.processMessage(alice.groupState, bobEncodedMessage)
    if (aliceProcessBob.kind === "newState") {
      throw new Error("Expected application message")
    }
    alice.groupState = aliceProcessBob.newState
    const aliceReceivedFromBob = new TextDecoder().decode(aliceProcessBob.message)
    ...
    const charlieCommitResult = await alice.marmot.createCommit(alice.groupState, [addCharlieProposal])
    ...
    const aliceGroupId = Buffer.from(alice.groupState!.groupContext.groupId).toString('hex')
    const bobGroupId = Buffer.from(bob.groupState!.groupContext.groupId).toString('hex')
    const charlieGroupId = Buffer.from(charlie.groupState!.groupContext.groupId).toString('hex')
...
async function advancedExample() {
  ...
  try {
    ...
    try {
      ...
      console.log("⚠️  Created credential with empty key (this might be valid depending on implementation)")
    } catch (error) {
      console.log("✅ Properly caught invalid credential error:", (error as Error).message)
    }
    ...
    const groupWithCustomId = await marmot.createGroup(testKeyPackage, customGroupId)

    const actualGroupId = Buffer.from(groupWithCustomId.groupContext.groupId).toString('hex')
    console.log(`✅ Created group with custom ID: ${actualGroupId.substring(0, 16)}...`)
