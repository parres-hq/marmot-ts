...
export interface ConformanceManifestEntry {
  id: string;
  kind: "scenario_vector" | "scenario" | "generated_scenario_family" | "formal_case_fixture" | "byte_fixture";
  status: "portable" | "incident_replay" | "rust_only_bridge" | "rust_only" | "generated" | "portable_policy_fixture";
  artifact?: string;
  coverage: string[];
  next: string;
}
...
function requireExactKeys(value: Record<string, unknown>, keys: string[], where: string): void {
...
export function resolveManifestArtifact(vectorsRoot: string, artifact: string): string {
  if (!artifact || path.isAbsolute(artifact)) throw new Error("artifact must be a relative path");
...
export function validateConformanceManifest(raw: unknown, vectorsRoot: string): ConformanceManifest {
  if (!isRecord(raw)) throw new Error("manifest must be an object");
  requireExactKeys(raw, MANIFEST_KEYS, "manifest");
  if (raw.manifest_version !== "1" || typeof raw.updated !== "string" || typeof raw.purpose !== "string")
    throw new Error("invalid manifest header");
  if (!Array.isArray(raw.entries)) throw new Error("manifest entries must be an array");
  const entries = raw.entries.map((candidate, index): ConformanceManifestEntry => {
    if (!isRecord(candidate)) throw new Error(`entry ${index} must be an object`);
    requireExactKeys(candidate, "artifact" in candidate ? ENTRY_KEYS : CATALOG_ENTRY_KEYS, `entry ${index}`);
    if (
      typeof candidate.id !== "string" ||
      !["scenario_vector", "scenario", "generated_scenario_family", "formal_case_fixture", "byte_fixture"].includes(String(candidate.kind)) ||
      !["portable", "incident_replay", "rust_only_bridge", "rust_only", "generated", "portable_policy_fixture"].includes(String(candidate.status)) ||
    ...
    if (candidate.kind === "scenario_vector" && typeof candidate.artifact === "string")
      resolveManifestArtifact(vectorsRoot, candidate.artifact);
    return candidate as unknown as ConformanceManifestEntry;
  });
  return { manifest_version: "1", updated: raw.updated, purpose: raw.purpose, entries };
}
